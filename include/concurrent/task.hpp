...
/// 任务节点：窃取队列与全局环上流转的实体。稳态由每 worker 空闲链表回收；
/// 队列槽中仅存指针（平凡可拷贝），保证 Chase-Lev 读-CAS 语义安全。
struct task_node {
    sbo_function<64> body;
    task_node* next_free = nullptr;
    std::size_t origin = 0; ///< 归属 worker 索引（执行完毕回收）
};

/// 共享状态自旋锁（临界区极短：续延链表头操作）
class spinlock {
public:
    void lock() noexcept {
...
public:
    struct guard {
        explicit guard(spinlock& s) noexcept
        : s_(s) { s_.lock(); }
        ~guard() { s_.unlock(); }
...
struct cont_impl : cont_node {
    cont_impl() noexcept {
        ...
        destroy = [](cont_node* self) noexcept {
...
public:
    ...
    /// 移动取值（恰好一次；外壳保证调用次序）。成员模板延迟实例化，避免 T=void 时形成 void&。
    template <typename U = T>
        requires(!std::is_void_v<U>)
    [[nodiscard]]
    ...
    [[nodiscard]]
    std::exception_ptr raw_exception() const noexcept { return exc_; }

    [[nodiscard]]
    std::atomic<std::uint32_t>& raw_done() noexcept { return done_; }

    void wait_done() const { done_.wait(0, std::memory_order_acquire); }
...
struct and_then_cont final : cont_impl<ParentState, and_then_cont<ParentState, F, InnerTask>> {
...
private:
    ...
    [[nodiscard]]
    std::tuple<Ts...> assemble(std::index_sequence<I...>)
...
public:
    ...
    /// 内部构造（池与组合子使用）
    explicit task(std::shared_ptr<detail::shared_state<T>> s) noexcept
    : st_(std::move(s)) {}
...
task<std::tuple<Ts...>> when_all(task<Ts>... ts) {
    if constexpr (sizeof...(Ts) == 0) {
        ...
        try {
            ...
            [[maybe_unused]] auto attach_one = [&]<std::size_t I, typename TIn>(
