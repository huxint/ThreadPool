...
namespace detail {
    ...
    template <typename T>
    inline constexpr bool is_priority_flag_v = std::is_same_v<std::remove_cv_t<T>, priority_flag>;
    template <typename T>
    inline constexpr bool is_cancellable_flag_v = std::is_same_v<std::remove_cv_t<T>, cancellable_flag>;
    template <typename T>
    inline constexpr bool is_trace_flag_v = std::is_same_v<std::remove_cv_t<T>, trace_flag>;
    ...
    template <typename... Flags>
    inline constexpr std::size_t worker_capacity_v = (std::max({worker_cap_value<Flags>::value..., std::size_t{0}}));
} // namespace detail
} // namespace concurrent
