...
零依赖的 header-only 线程池, 面向两类场景: 追求低延迟与高吞吐的任务流(唤醒路径有界自旋, 空池往返亚微秒级), 以及需要错误安全的服务型代码(库自身失败走 `std::expected`, 任务体异常透传至结果通道, 不泄漏到池外). 任务以 128B 节点 + SBO 闭包承载, 外部提交进无锁全局队列, worker 内嵌套提交进本地 deque, 提交永不阻塞永不拒绝.
...
如实说: 单生产者与稀疏流量(唤醒路径主导)是本库的强项, 延迟更是独一档; 递归 fork-join 与 oneTBB 同级(分片记账: 每线程一个 pending 单元, 占坑/完成双方零共享行 RMW; work-first 派生: `fork_join`, 每层只付一次提交通道), 剩余差距来自任务表示 — TBB 任务几十字节且经每线程池回收, 本库每层派生一次 128B 节点 + 闭包构造; 8 生产者饱和流量下 moodycamel 自建池约快一倍(剩余成本在 Vyukov 环的每推/每弹 RMW、每提交一道 seq_cst 栅栏与 4 MiB 环的缓存足迹). 分块映射与 TBB 基本持平. 属已知的结构性差距而非缺陷; 高生产者饱和流量可自行改用 `queue_cap` 下调环体积或走批量提交.
