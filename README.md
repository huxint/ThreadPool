...
|------|---------:|----------------:|-----:|------------:|
| fire-and-forget 单生产者 | 1.44 | 0.37 | **5.02** | **3.49x** |
| submit + 取回结果 | 0.35 | 0.25 | **2.04** | **5.83x** |
| 递归 fork-join(深度 18) | 4.53 | 0.64 | **22.67** | **5.00x** |
| 多生产者竞争 x2 | 2.81 | 0.43 | **6.25** | **2.22x** |
| 多生产者竞争 x4 | 2.80 | 0.69 | **5.95** | **2.13x** |
| 多生产者竞争 x8 | 2.66 | 1.33 | **5.14** | **1.93x** |
...
延迟是本库优势最稳的一面: 12 次运行中三个分位无一例外, 且本库的离散度也最小(P50 跨运行区间 0.47-0.60µs). 单次最大值(max)三方都受调度抖动支配, 跨运行可差一个数量级, 不列入表内

混合负载与扩展性:

| 场景(ms, 越小越好) | Taskflow | BS::thread_pool | 本库 |
|------|---------:|----------------:|-----:|
| 混合负载: 90% 短任务 + 10% 长任务 | 15.64 | 11.93 | **10.16** |
| 扩展性: 固定实算 x 1 线程 | 13.25 | **12.75** | 12.43 |
| 扩展性: 固定实算 x 2 线程 | **7.16** | 7.19 | 7.94 |
| 扩展性: 固定实算 x 4 线程 | **3.98** | 4.21 | 4.19 |
| 扩展性: 固定实算 x 8 线程 | **2.19** | 2.22 | 2.39 |

扩展性一项由任务体实算量支配, 调度开销被摊薄到可忽略, 因此三方基本持平(各行跨运行区间互相重叠, 中位数差异最大为 2 线程行的 11%). 本库的调度优势只在任务粒度小到调度开销可比时才体现 — 即上面的吞吐与延迟两表

特性组合吞吐(execute 单生产者): 各特性标签单独开启及全开时的相对吞吐:

| 标签组合 | 吞吐(M/s) | 相对无标签 | 跨运行离散度 |
|----------|--------:|-----------:|------------:|
| 无标签 | 4.59 | 1.00x | cv 12% |
| + priority | 4.81 | 1.05x | cv 7% |
| + cancellable | 4.50 | 0.98x | cv 9% |
| + trace 未设钩子 | 5.66 | 1.23x | cv 4% |
| + trace on_end 空钩子 | 5.59 | 1.22x | cv 4% |
| + worker_cap&lt;8&gt; | 4.45 | 0.97x | cv 8% |
| 全部组合 | 5.45 | 1.19x | cv 6% |
...
|------|-------:|--------------:|-----:|
| fire-and-forget 单生产者(M/s) | 3.7–4.5 | 2.4–4.3 | **4.4–5.1** |
| 8 生产者竞争(M/s) | 6.9–7.3 | **9.6–10.4** | 3.8–5.9 |
| 递归 fork-join 深度 18(M leaves/s) | **20.0–29.8** | 3.8–4.2 | 19.6–24.8 |
| 分块并行映射 64 grain(ms, 越小越好) | **2.9–4.0** | — | 3.1–4.3 |

如实说: 单生产者与稀疏流量(唤醒路径主导)是本库的强项, 延迟更是独一档; 递归 fork-join 曾落后 oneTBB 约 4 倍, 分片记账(每线程一个 pending 单元, 占坑/完成双方零共享行 RMW)落地后已追至与 TBB 同级或接近, 剩余差距来自任务表示 — TBB 任务几十字节且经每线程池回收, 本库每层派生一次 128B 节点 + 闭包构造; 8 生产者饱和流量下 moodycamel 自建池仍约快一倍(剩余成本在 Vyukov 环的每推/每弹 RMW、每提交一道 seq_cst 栅栏与 4 MiB 环的缓存足迹, 均与记账无关). 分块映射与 TBB 基本持平. 属已知的结构性差距而非缺陷; 高生产者饱和流量可自行改用 `queue_cap` 下调环体积或走批量提交.
