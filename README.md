...
|------|---------:|----------------:|-----:|------------:|
| fire-and-forget 单生产者 | 1.61 | 0.41 | **5.56** | **3.45x** |
| submit + 取回结果 | 0.37 | 0.26 | **1.73** | **4.68x** |
| 递归 fork-join(深度 18) | 4.53 | 0.69 | **5.88** | **1.30x** |
| 多生产者竞争 x2 | 2.98 | 0.46 | **5.23** | **1.75x** |
| 多生产者竞争 x4 | 2.91 | 0.75 | **5.44** | **1.87x** |
| 多生产者竞争 x8 | 2.60 | 1.44 | **5.27** | **2.02x** |
...
延迟是本库优势最稳的一面: 12 次运行中三个分位无一例外, 且本库的离散度也最小(P50 跨运行区间 0.55-0.65µs). 单次最大值(max)三方都受调度抖动支配, 跨运行可差一个数量级, 不列入表内

混合负载与扩展性:

| 场景(ms, 越小越好) | Taskflow | BS::thread_pool | 本库 |
|------|---------:|----------------:|-----:|
| 混合负载: 90% 短任务 + 10% 长任务 | 13.75 | 11.21 | **9.69** |
| 扩展性: 固定实算 x 1 线程 | 13.00 | **11.44** | 13.07 |
| 扩展性: 固定实算 x 2 线程 | 8.18 | **7.23** | 8.01 |
| 扩展性: 固定实算 x 4 线程 | 3.83 | 4.11 | **3.74** |
| 扩展性: 固定实算 x 8 线程 | **2.08** | 2.09 | 2.31 |

扩展性一项由任务体实算量支配, 调度开销被摊薄到可忽略, 因此三方基本持平(差异 ±10%, 且 1/2 线程行跨运行区间互相重叠). 本库的调度优势只在任务粒度小到调度开销可比时才体现 — 即上面的吞吐与延迟两表

特性组合吞吐(execute 单生产者): 各特性标签单独开启及全开时的相对吞吐:

| 标签组合 | 吞吐(M/s) | 相对无标签 | 跨运行离散度 |
|----------|--------:|-----------:|------------:|
| 无标签 | 5.74 | 1.00x | cv 11% |
| + priority | 5.18 | 0.90x | cv 16% |
| + cancellable | 5.29 | 0.92x | cv 12% |
| + trace 未设钩子 | 5.01 | 0.87x | cv 3.5% |
| + trace on_end 空钩子 | 4.91 | 0.85x | cv 3.2% |
| + worker_cap&lt;8&gt; | 5.15 | 0.90x | cv 10% |
| 全部组合 | 5.69 | 0.99x | cv 6.8% |

> 此表不宜按严格序关系解读: 除 trace 两行外, 各行的跨运行离散度(cv 10-16%)大于行间差值, "全部组合"反而快于单开 priority 即为噪声所致. 能站住的结论只有两条 — trace 有约 13-15% 的实际代价(其两行 cv 仅 3%, 且 12 次运行从未超过 5.22 M/s, 而其余组合可达 6.5-7.4); priority / cancellable / worker_cap 的开销小到当前样本量无法分辨
